...
from .utils import send_orderNo
        ...
        if password == password2 :
            if len(password) >= 8:
                if User.objects.filter(email=email).exists():
                    ...
                    send_orderNo(user.email)
                    user.save()

                    user_db = User.objects.get(email=email,first_name=firstname)

                    return render(request, 'thank.html',{'user_db':user_db})
            else:
